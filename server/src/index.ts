...
app.get("/admin/orders", (req: Request, res: Response) => {
  try {
    // Get all orders with user details and addresses
    const orders = db.prepare(`
      SELECT 
        o.id, 
        o.status, 
    ...
    const result = orders.map((order) => ({
      ...order,
      order_items: itemsStmt.all(order.id),
    }));
...
app.put("/admin/orders/:id/status", (req: Request, res: Response) => {
  try {
    const orderId = req.params.id;
    const { status } = req.body as { status: string };

    if (!status || !["placed", "in_progress", "delivered"].includes(status)) {
      return res.status(400).json({ error: "Invalid status. Must be 'placed', 'in_progress', or 'delivered'" });
    }
...
app.delete("/admin/orders/:id", (req: Request, res: Response) => {
  try {
    const orderId = req.params.id;
    ...
    if (order.status !== "delivered") {
      return res.status(400).json({ error: "Can only delete delivered orders" });
    }
...
if (fs.existsSync(clientDistPath)) {
  ...
  const spaRoutes = ["/", "/shop", "/about", "/contact", "/cart", "/checkout", "/auth", "/admin/login", "/admin/dashboard"];
